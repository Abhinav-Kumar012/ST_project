#!/bin/bash

# unit tests mutation commands
mut.py --target src.geometry --unit-test tests.test_geometry --operator SDL LCR COD --path . --report-html Reports/unit_geometry
mut.py --target src.banking --unit-test tests.test_banking --operator SDL LCR COD --path . --report-html Reports/unit_banking
mut.py --target src.graph_algos --unit-test tests.test_graphs --operator SDL LCR COD --path .  --report-html Reports/unit_graph
mut.py --target src.data_structures --unit-test tests.test_data_structures --operator SDL COD COI --path . --report-html Reports/unit_data_structures
mut.py --target src.matrix_ops --unit-test tests.test_matrix --operator SDL LCR COD --path . --report-html Reports/unit_matrix_ops
mut.py --target src.search_algos --unit-test tests.test_search_algos --operator SDL LCR COI --path . --report-html Reports/unit_search_algos
mut.py --target src.set_ops --unit-test tests.test_set_ops --operator SDL LCR COD --path . --report-html Reports/unit_set_ops
mut.py --target src.sorting_algos --unit-test tests.test_sorting_algos --operator SDL LCR COI --path . --report-html Reports/unit_sorting_algos
mut.py --target src.stats_lib --unit-test tests.test_stats_lib --operator SDL LCR COD --path . --report-html Reports/unit_stats_lib
mut.py --target src.string_utils --unit-test tests.test_string_utils --operator SDL LCR COD --path . --report-html Reports/unit_string_utils
mut.py --target src.utils --unit-test tests.test_utils_2 --operator SDL LCR COD --path . --report-html Reports/unit_utils

# integration tests mutation commands
mut.py --target src.integration --unit-test tests.test_integration --operator SDL LCR COI ROR --path . --report-html Reports/integration_1
mut.py --target src.integration_2 --unit-test tests.test_integration_2 --operator SDL LCR COI ROR --path . --report-html Reports/integration_2
mut.py --target src.integration_3 --unit-test tests.test_integration_3 --operator SDL LCR COI ROR --path . --report-html Reports/integration_3